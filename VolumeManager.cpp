...
void VolumeManager::readInitialState() {
...
#ifdef TARGET_IS_LEPHONE
            mUsbMassStorageEnabled = !strncmp(state, "online", 6);
#else
            mUsbMassStorageEnabled = !strncmp(state, "1", 1);
#endif
        } else {
#ifdef TARGET_IS_LEPHONE
            SLOGE("Failed to read usb_mass_storage switch (%s)", strerror(errno));
#else
...
#ifdef TARGET_IS_LEPHONE
        SLOGD("usb_mass_storage switch is not enabled in the kernel");
#else
     ...
     */
#ifdef TARGET_IS_LEPHONE
    if ((fp = fopen("/sys/devices/virtual/switch/msm_hsusb_peripheral/state", "r"))) {
#else
...
#ifdef TARGET_IS_LEPHONE
            SLOGE("Failed to read msm_hsusb_peripheral switch (%s)", strerror(errno));
#else
...
#ifdef TARGET_IS_LEPHONE
        SLOGD("msm_hsusb_peripheral switch is not enabled in the kernel");
#else
...
void VolumeManager::handleSwitchEvent(NetlinkEvent *evt) {
...
#ifdef TARGET_IS_LEPHONE
    if (!strcmp(name, "msm_hsusb_peripheral")) {
    ...
    if (!strcmp(name, "usb_configuration")) {
        ...
        mUsbMassStorageEnabled = !strcmp(state, "online");
